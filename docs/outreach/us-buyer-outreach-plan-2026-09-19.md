...
- 发信身份：当前只有个人 Gmail 和接收转发型企业邮箱。接收转发型企业邮箱通常不能作为稳定发信邮箱使用；正式外联前建议配置真正可发送的域名邮箱，并完成 SPF、DKIM、DMARC。个人 Gmail 可用于内部测试或极小样本人工沟通，但不建议作为长期 B2B 主动开发发信身份。
...
4. 是否使用 `info@nixiafabric.com` 或其他已确认的公司邮箱作为发信身份。
5. 是否要先做 5 家小样本人工审核，再进入每周 20-30 家节奏。
